...
The first thing we are going to do is import the data:
```{r import}
...
d1_raw <- d1 %>% #Creating pid
  ...
  #I would recommend using the variable names here at least a little, so that it is clearer as a tutorial what you are selecting? 
  
  #Select (just-created) pid and group, Duration in seconds and RecordedDates, and then 107 columns.
  rename(date = RecordedDate,
         ...
         age = q348) %>% 
  mutate(age = (age + 15)) %>% 
  select(-7) #Omits gender_text column (again I would consider using the name of the column to make that clearer)
...
map_dbl(by_gender$data, ~max(.x$q352_1, na.rm = TRUE)) #Note from KH: I need to figure out why min and max is yielding "NA" for gender category 2. #AAC: I fixed it, Katherine! :D 
