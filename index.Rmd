---
title: "Final Project- Functional Programming with R"
output: 
  flexdashboard::flex_dashboard:
    orientation: column
    source_code: "https://github.com/alexisadamsclark/Final_Project_RCourse3.git"
...
knitr::opts_chunk$set(message = FALSE, error = FALSE, warning = FALSE, fig.width = 5, fig.height = 20)
```
...
--------
Who is behind this beautiful flex dashboard tutorial? 
\
\
Alexis Adams-Clark
\
...
```{r tidy, echo = TRUE, fig.height=500}
d1_raw <- d1 %>% #Creating pid
  mutate(pid = row_number(),
         group = 1) %>% 
  select(115:116, 3:4, 7:114) %>% #Select variables and columns
  #Select (just-created) pid and group, 
  #Duration in seconds and RecordedDates, and then 107 columns.
...
d2_raw <- d2 %>%
  mutate(pid = (row_number() + 437), 
         group = 2) %>% 
  select(119, 120, everything()) %>% #Moves column 119 and 120
  rename(date = RecordedDate,
         duration = Duration__in_seconds_,
         race_ethn = q6,
         gender = q62,
...
```{r tsc_new, echo = TRUE}
#Writing a function - find means of each item of tsc
...
### Walk Code
```{r walk, echo = TRUE, eval = FALSE}
tsc_split <- split(tsc_pid, tsc_pid$gender)
gender_names <- c("female", "male", "nonbinary", "self-described", "declined to state")

plots <- map2(tsc_split, gender_names, ~ ggplot(.x, aes(x = tsc_sum)) + 
          geom_bar(fill = "cornflower blue", color = "white") +
          labs(title = glue::glue("Distribution of scores from {.y} gender")))
...
### Plot 1
```{r walk2, fig.width=8, fig.height=5}
tsc_split <- split(tsc_pid, tsc_pid$gender)
gender_names <- c("female", "male", "nonbinary", "self-described", "declined to state")
...
### Plot 2
```{r walk22, fig.width=8, fig.height=5}
plots$`2`
```


### Plot 3
```{r walk3, fig.width=8, fig.height=5}
plots$`3`
```

### Plot 4
```{r walk4, fig.width=8, fig.height=5}
plots$`4`
```

### Plot 5
```{r walk5, fig.width=8, fig.height=5}
plots$`5`
```
...
#By nesting, we can ask a number of questions about groups within the data. 
#For example we can see how many participants endorsed each gender category:
d1_raw %>%
  nest(-gender) %>%
...
map_dbl(by_gender$data, ~max(.x$q352_1, na.rm = TRUE))
```
